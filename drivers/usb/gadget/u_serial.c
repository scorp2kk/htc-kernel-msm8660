...
static int gs_poll_wr_req_complete = 0;
/**
...
static void gs_poll_write_complete(struct usb_ep *ep,
	...
	gs_poll_wr_req_complete = 1;
	pr_vdebug("%s", __func__);
...
static int __gs_poll_put_char(struct gs_port *port, char ch) {
	...
	pr_vdebug("%s: polling for write (ep=0x%08x)\n", __func__, (unsigned int)ept );

	while ( !gs_poll_wr_req_complete ) {
		usb_loop_poll_hw();
	}
