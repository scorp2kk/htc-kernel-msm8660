...
static void mipi_novatek_lcd_setup(int panel, struct platform_device *pdev)
{
	int i;
	int err_count;
	uint8_t err_seen;
...
static int mipi_novatek_lcd_on(struct platform_device *pdev)
{
    ...
    if (panel_type != PANEL_ID_NONE) {
      PR_DISP_INFO("%s: %s\n", __func__, ptype);
      mipi_novatek_lcd_setup(panel_type, pdev);
...
static int mipi_novatek_lcd_off(struct platform_device *pdev)
{
	...
	mipi_dsi_cmds_tx(mfd, &novatek_tx_buf, mipi_power_off_cmd, mipi_power_off_cmd_size);
